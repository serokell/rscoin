...
import           Network.MessagePack.Client (clearLRUMap)
...
runRealMode bankHost rpcMonad = bracket
    (emptySettings bankHost)
    (clearLRUMap . connectionMap)
    (\settings -> runTimedIO $ flip runReaderT settings $ runMsgPackRpc rpcMonad)
